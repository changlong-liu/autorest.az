            ...
            for (let template of templates) {
                ...
                for (let depend of depends) {
                    if (depend == "name") {
                        ...
                        for (let i=0; i<preparerInfos[preparerInfo.className].dependParameters.length; i++) {
                            if (model.GetResourcePool().isResource(preparerInfos[preparerInfo.className].dependResources[i], null)==model.GetResourcePool().isResource(depend, null)) {
                                variables.push(`self.test_class_instance.kwargs.get(self.${preparerInfos[preparerInfo.className].dependParameters[i]})`);
                            }
                        }  
                    }
                }
